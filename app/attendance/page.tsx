...
export default function AttendancePage() {
  ...
  // ✨ FUNGSI UNTUK CETAK PDF PER HARI
  const handlePrintPDF = (date: string) => {
    setPrintDate(date);
    // Beri jeda sedikit agar React merender state yang disembunyikan sebelum memanggil print
    setTimeout(() => {
      window.print();
      setPrintDate(null); // Kembalikan ke tampilan normal setelah print dialog ditutup
    }, 150);
  };

  // ✨ MENGELOMPOKKAN DATA BERDASARKAN TANGGAL (GROUPING)
  const groupedAttendances = attendances.reduce((acc, curr) => {
    const dateObj = new Date(curr.created_at);
    // Format: "Selasa, 18 Agustus 2026"
  ...
  return (
    <main className="min-h-screen bg-gray-50/50 print:bg-white">
      {/* Header Utama - Disembunyikan saat di-print */}
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-8 print:p-0">
        ...
        {/* Title Section - Disembunyikan saat di-print */}
            ...
            <table className="w-full text-left text-sm">
              ...
              <thead className="bg-gray-50 text-gray-600 print:bg-transparent print:border-b-2 print:border-black">
                <tr>
                  ...
                  <th className="px-6 py-5 font-bold uppercase tracking-wider text-xs">Lokasi GPS</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {Object.keys(groupedAttendances).length === 0 ? (
                  ...
                  // LOOPING PER TANGGAL (SEKAT)
                  Object.keys(groupedAttendances).map((dateKey) => {
                    // Logika Print: Sembunyikan tanggal lain jika sedang nge-print tanggal tertentu
                        ...
                        <tr className={`bg-gradient-to-r from-blue-50/50 to-transparent border-t-2 border-blue-100 ${isHiddenDuringPrint}`}>
                          <td colSpan={7} className="px-6 py-4">
                            <div className="flex items-center justify-between">
                              ...
                              {/* TOMBOL CETAK PDF (Sembunyi saat diprint) */}
                              <button 
                          ...
                          return (
                            <tr key={item.id} className={`hover:bg-blue-50/30 transition-colors ${isHiddenDuringPrint} print:break-inside-avoid`}>
                              ...
                              <td className="px-6 py-5">
                                ...
                                <div className="text-[10px] font-semibold text-gray-400 mt-0.5 uppercase tracking-wider">{item.status === 'present' ? 'Hadir' : 'Telat'}</div>
                              </td>

                              {/* Kolom Foto (Sembunyi saat print agar rapi) */}
                              ...
                              <td className="px-6 py-5">
                                <div className="flex flex-col gap-1.5">
                                  <div className="flex items-center gap-2">
                                    <span className="inline-block w-8 text-xs font-bold text-gray-400">IN:</span>
                              ...
                              {/* ✨ Kolom Lokasi GPS Terpisah (Keren) */}
                              <td className="px-6 py-5">
                              ...
                              {/* Kolom Aksi (Approve/Reject) - Sembunyi saat print */}
