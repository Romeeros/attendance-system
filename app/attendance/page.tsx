...
export default function AttendancePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (employeeError) {
          console.error("Gagal mengambil karyawan:", employeeError.message);
          return;
        }
        ...
        const merged = (data || []).map((item) => {
          const employee = employees?.find((e) => e.id === item.profile_id);
  ...
  const handleApproval = async (
    ...
    if (!confirm(`Apakah Anda yakin ingin ${action} absensi ini?`)) return;
  ...
  const handlePrintPDF = (date: string) => {
    ...
    setTimeout(() => {
      window.print();
      setTimeout(() => setPrintDate(null), 300);
    }, 150);
  };
  ...
  const grouped = useMemo<GroupedData>(() => {
    ...
    attendances.forEach((item) => {
      ...
      const division = item.profiles?.division || "Tanpa Divisi";

      if (!result[date]) result[date] = {};
      if (!result[date][division]) result[date][division] = [];
        ...
        {!dates.length ? (
          ...
          <div className="space-y-10">
            ...
            {dates.map(([date, divisions]) => {
              const hidden = printDate && printDate !== date
                ? "print:hidden"
                : "";
              ...
              return (
                <section key={date} className={hidden}>
                  ...
                  <div className="space-y-5">
                    ...
                    {divisionEntries.map(([division, items]) => {
                                      ...
                                      <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-xs font-black text-blue-600">
                                            {initial(item.profiles?.full_name)}
                                          </div>
